...
function encodeToRot13(str) {
    let inputABC = 'ABCDEFGHIJKLMNOPQRSTUVWXYZabcdefghijklmnopqrstuvwxyz';
    let outputABC = 'NOPQRSTUVWXYZABCDEFGHIJKLMnopqrstuvwxyzabcdefghijklm';
    let index;
    let arr = str.split('');

    for (let i = 0; i < arr.length; i++) {
       if (inputABC.indexOf(arr[i]) != -1) {
        index = inputABC.indexOf(arr[i]);
        arr[i] = outputABC[index] ;
       }
    }
